...
var strokeFolder = gui.addFolder('stroke color');
...
var increase = wave.frequency;
function animate() {
  c.fillStyle = "rgba(".concat(backColor.r, ", ").concat(backColor.g, ", ").concat(backColor.b, ", ").concat(backColor.a, ")");
  c.fillRect(0, 0, canvas.width, canvas.height);
  requestAnimationFrame(animate);
  c.beginPath();
  c.moveTo(0, canvas.height / 2);
  for (var i = 0, j = 0; i < canvas.width; i++, j++) {
    c.lineTo(i, wave.y + Math.sin(i * wave.length + increase) * wave.amplitude * Math.sin(increase * j));
    j *= Math.sin(j) * 0.005 + 0.5;
  }
  c.strokeStyle = "hsl(".concat(Math.abs(strokeColor.h * Math.sin(increase)), ", ").concat(strokeColor.s, "%, ").concat(strokeColor.l, "%)");
  c.stroke();
